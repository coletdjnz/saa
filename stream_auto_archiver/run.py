...
class LoggingSHandler(logging.StreamHandler):
    def __init__(self):
...
class StreamArchiver:
    ...
    def __init__(self):
        ...
        self.log_name = f"[{self.stream_name}]"
    ...
    def _check_live(self, url: str):
        ...
        try:
            ...
            log.error(f"{self.log_name} JSONDecodeError while trying to check if stream is live or not (probably an issue with Streamlink): {e}")
            return False
        ...
        if "error" not in json_output:
            ...
            log.debug(f"{self.log_name} Streamlink said: {json_output['error']}")
            return False
    ...
    def _download_handler(self, stream, directory):
        ...
        while True:
            ...
            log.info(f"{self.log_name} Beginning Stream Download of {filename}")
            ...
            if status == 0:
                log.info(f"{self.log_name} Cutting stream")
                self.current_process.kill()

            log.info(f"{self.log_name} Finalizing files...")
            ...
            if status > 0:
                # Stream has ended
                if status == 1:
                    log.info(f"{self.log_name} Stream has ended.")
                    return 1
        ...
        while (time.time() - start_time) <= self.split_time:
            ...
            for line in self.stdout_data[prev_output:]:
                ...
                if "failed to reload playlist: unable to open url" in line.lower():
                    log.debug(f"{self.log_name} Stream has probably ended - Streamlink said: {line}")
                    break

                log.error(f"{self.log_name} {line}")
    ...
    def site_watchdog(self):
        ...
        while run:
            ...
            if not stream_status:
                if not_live_runs == 0:
                    log.info(f"{self.log_name} {self.stream_name} is not currently live.")
                    not_live_runs += 1
                else:
                    log.debug(f"{self.log_name} {self.stream_name} is not currently live.")
            else:
                log.info(f"{self.log_name} {self.stream_name} is live, archiving started.")
                not_live_runs = 0
    ...
    def cleanup(self):
        ...
        log.info(f"{self.log_name} Cleaned up {total_cleaned} unfinished streams.")
